(function () {
	...
	function rating_kp_imdb(card) {
		...
		function searchFilm() {
			var url = params.url;
			var url_by_title = Prisma.Utils.addUrlComponent(url + 'api/v2.1/films/search-by-keyword', 'keyword=' + encodeURIComponent(clean_title));
			if (card.imdb_id) url = Prisma.Utils.addUrlComponent(url + 'api/v2.2/films', 'imdbId=' + encodeURIComponent(card.imdb_id));
		...
		function showError(error) {
			Prisma.Noty.show('Ошибка в отображении рейтинга: ' + error);
		}

		function _getCache(movie) {
			var timestamp = new Date().getTime();
			var cache = Prisma.Storage.cache('kp_rating', 500, {}); //500 это лимит ключей
			if (cache[movie]) {
				if ((timestamp - cache[movie].timestamp) > params.cache_time) {
					// Если кеш истёк, чистим его
					delete cache[movie];
					Prisma.Storage.set('kp_rating', cache);
					return false;
				}
			} else return false;
			return cache;
		...
		function _showRating(data) {
			if (data) {
				...
				var render = Prisma.Activity.active().activity.render();
